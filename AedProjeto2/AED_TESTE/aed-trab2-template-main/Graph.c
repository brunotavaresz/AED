...
Graph* GraphFromFile(FILE* f) {
  ...
  if (result != 3) {
    fprintf(stderr, "Error reading graph properties from file.\n");
    return NULL;
  }
  ...
  unsigned int v, w;
  double weight;
  while (fscanf(f, "%u %u %lf", &v, &w, &weight) == 3) {
    if (isWeighted) {
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  for (unsigned int i = 0; i < ListGetSize(vertex->edgesList); ListMoveToNext(vertex->edgesList), i++) {
    ...
    if (edge->adjVertex == w) {
      ...
      if (g->isDigraph == 0) {
        // Remove the corresponding edge in the other direction
        ListMove(destVertex->edgesList, v);
        for (unsigned int j = 0; j < ListGetSize(destVertex->edgesList); ListMoveToNext(destVertex->edgesList), j++) {
          struct _Edge* reverseEdge = ListGetCurrentItem(destVertex->edgesList);
...
int GraphCheckInvariants(const Graph* g) {
  ...
  for (; i < g->numVertices; ListMoveToNext(vertices), i++) {
    ...
    // Verificar invariante: o grau de entrada é igual ao número de arestas de entrada
    assert(v->inDegree == ListGetSize(v->edgesList));

    // Verificar invariante: o grau de saída é igual ao número de arestas de saída
    assert(v->outDegree == ListGetSize(v->edgesList));
  }
